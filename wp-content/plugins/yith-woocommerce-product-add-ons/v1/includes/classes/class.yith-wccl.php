...
if ( ! class_exists( 'YITH_WCCL' ) ) {
	...
	class YITH_WCCL {
		...
		public function is_admin() {
			$actions     = apply_filters( 'yith_wccl_is_admin_actions_array', array( 'prdctfltr_respond_550', 'flatsome_quickview' ) );
			...
			$is_ajax     = defined( 'DOING_AJAX' ) && DOING_AJAX && ( $is_frontend || ( isset( $_REQUEST['action'] ) && in_array( $_REQUEST['action'], $actions, true ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		 ...
		 * @param string $is_keep_existing_images Is keep existing images.
		 */
		public function wpai_import_gallery_images_variation( $post_id, $att_id, $filepath, $is_keep_existing_images = '' ) {
			...
			if ( ! in_array( $att_id, $gallery, true ) && ( ! empty( $featured_image ) && $featured_image !== $att_id ) ) {
				$gallery[] = $att_id;
		...
		public function product_export_meta_value( $value, $meta, $product, $row ) {
			if ( '_yith_wccl_gallery' === $meta || empty( $value ) ) {
				return $value;
			}
 ...
 * @return \YITH_WCCL
 */
function YITH_WCCL() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
