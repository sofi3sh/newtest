	...
	function yith_wapo_init() {
		if ( ! apply_filters( 'yith_wapo_disable_init', false ) ) {
			if ( function_exists( 'WC' ) ) {
				require_once YITH_WAPO_DIR . 'v1/includes/classes/class.yith-wapo-group.php';
				require_once YITH_WAPO_DIR . 'v1/includes/classes/class.yith-wapo-option.php';
				require_once YITH_WAPO_DIR . 'v1/includes/classes/class.yith-wapo-settings.php';
				require_once YITH_WAPO_DIR . 'v1/includes/classes/class.yith-wapo-type.php';
				require_once YITH_WAPO_DIR . 'v1/includes/classes/class.yith-wapo.php';
				if ( defined( 'YITH_WAPO_PREMIUM' ) && YITH_WAPO_PREMIUM ) {
					require_once YITH_WAPO_DIR . 'v1/includes/classes/class.yith-wapo-premium.php';
				}
...
if ( ! function_exists( 'yith_wapo_install_woocommerce_admin_notice' ) ) {
	...
	function yith_wapo_install_woocommerce_admin_notice() { ?>
		<div class="error">
			<p>YITH WooCommerce Product Add-Ons <?php echo esc_html__( 'is enabled but not effective. It requires WooCommerce in order to work.', 'yith-woocommerce-product-add-ons' ); ?></p>
		</div>
		<?php
	}
}
...
if ( ! function_exists( 'yith_wapo_multi_products_select' ) ) {
	 ...
	 * @param array  $selected_products Selected products.
	 */
...
if ( ! function_exists( 'yith_wapo_currency_check' ) ) {
	...
	function yith_wapo_currency_check( $price ) {
		...
		if ( class_exists( 'WOOCS' ) ) {
			...
			$currencies      = $WOOCS->get_currencies(); // phpcs:ignore WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase
			$conversion_rate = $currencies[ $WOOCS->current_currency ]['rate']; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase
