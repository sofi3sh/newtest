...
if ( ! function_exists( 'yith_wapo_get_addons_by_group_id' ) ) {
	...
	function yith_wapo_get_addons_by_group_id( $group_id ) {
		...
		$results           = $wpdb->get_results( "SELECT id FROM {$wpdb->prefix}$addons_table_name WHERE group_id='$group_id' AND del='0'" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$addons_array      = array();
...
if ( ! function_exists( 'yith_wapo_get_addons_number_by_group_id' ) ) {
	...
	function yith_wapo_get_addons_number_by_group_id( $group_id ) {
		global $wpdb;
		$addons_table_name = YITH_WAPO_Type::$table_name;
		return $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}$addons_table_name WHERE group_id='$group_id' AND del='0'" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	}
}
